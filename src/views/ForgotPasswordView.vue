<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <p class="auth-footer-text">
        Already have a code?
        <RouterLink :to="{ name: 'reset-password', query: email.value ? { email: email.value } : {} }">Enter it here</RouterLink>
      </p>
    </div>
  </div>
</template>
...
async function handleSubmit() {
  ...
  try {
    ...
    // Swallow API errors to prevent user enumeration.
    // Re-throw genuine network/connectivity errors so the user is not misled.
    if (!err.status) {
      loading.value = false
  ...
  successMessage.value = 'If an account exists for that email, a reset code has been sent. Redirecting…'
  setTimeout(() => router.push({ name: 'reset-password', query: { email: email.value } }), 2000)
}
</script>
